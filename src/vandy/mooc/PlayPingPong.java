...
public class PlayPingPong implements Runnable {
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        public boolean handleMessage(Message reqMsg) {
        	...
        	PingPong pingPong = PingPong.values()[reqMsg.what];
       
            if (mIterationsCompleted < mMaxIterations) {
            	
             int currentIteration = mIterationsCompleted + 1;
            mOutputStrategy.print(pingPong.name() + "(" + currentIteration + ")");
        	...
        	PingPong ppType;
        	if (pingPong.equals(PingPong.PING))
        		ppType = PingPong.PONG;
        	else
        		ppType = PingPong.PING;
        	
        	      	
        	Message msg = 
        			replyTohandler.obtainMessage(
        					ppType.ordinal(), mHandlers[ppType.ordinal()]);
